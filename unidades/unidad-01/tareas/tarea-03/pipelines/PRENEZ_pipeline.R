...
# Configuración
# Usamos el path exacto del archivo PDF general
input_pdf <- "unidades/unidad-01/tareas/tarea-03/Input/TEST DE PREÑEZ.pdf"
output_csv <- "unidades/unidad-01/tareas/tarea-03/data-procesada/PRENEZ_LIMPIO.csv"

# Leer PDF
# pdftools extrae el texto manteniendo la estructura visual aproximada
text <- pdf_text(input_pdf)
lines <- unlist(strsplit(text, "\n"))

results <- list()

cat("Iniciando procesamiento de Test de Preñez (General)...\n")
...
for (line in lines) {
  ...
  # Animal Nr.Reg Estatus GP Lact_Nr Lact_Dias Cant_Ins Fecha_Ins Dias_Ins IEP
...
# Eliminamos duplicados por si acaso hay solapamiento en el PDF
df_final <- bind_rows(results) %>% distinct()

# Guardamos con BOM para que Excel no tenga problemas con caracteres raros
